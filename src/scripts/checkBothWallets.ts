...
const checkBothWallets = async () => {
    ...
    const [address1Arg, address2Arg] = process.argv.slice(2);
    const ADDRESS_1 = address1Arg || PROXY_WALLET;
    const ADDRESS_2 = address2Arg || '';

    if (!ADDRESS_2) {
        console.log('Missing second address.\n');
        console.log(
            'Usage: npx ts-node src/scripts/checkBothWallets.ts <address_1> <address_2>\n'
        );
        return;
    }
